from datetime import datetime

from src.data_object.trade_order import MarketTradeOrder
from src.enums.order_enum import OrderSide, VolumeType, AssetOrderType
from src.enums.watch_list_enum import WatchList
from src.utils import logger, assert_util
from src.utils.assert_util import verify_equals
from src.utils.string_util import format_number_string, count_decimal_places


def test_place_market_order_with_stop_loss_and_take_profit(web, symbol):
    logger.info(f"Step 1: Select {symbol!r}")
    web.trade_page.select_watchlist(WatchList.CRYPTO)
    web.trade_page.select_symbol(symbol)
    web.trade_page.place_order.collapse_trade_details()

    logger.info("Step 2: Get live buy price")
    live_buy_price = float(format_number_string(web.trade_page.place_order.get_live_buy_price()))
    decimal_places = count_decimal_places(str(live_buy_price))

    logger.info("Step 3: Fill in buy order info")
    trade_order = MarketTradeOrder(OrderSide.BUY, {VolumeType.UNITS: 1}, live_buy_price - 5, live_buy_price + 5)
    web.trade_page.place_order.place_an_order(trade_order, False)

    verify_equals(
        web.trade_page.trade_confirmation_popup.get_confirmation_symbol(), symbol,
        f"Verify {symbol!r} is correct in confirmation popup"
    )

    actual_trade_order_info = web.trade_page.trade_confirmation_popup.get_confirmation_trade_order_info()
    trade_order.to_json()
    expected_values = [
        ("order_side", trade_order.order_side),
        ("volume", str(trade_order.get_volume_value())),
        ("stop_loss", f"{trade_order.stop_loss:,.{decimal_places}f}"),
        ("take_profit", f"{trade_order.take_profit:,.{decimal_places}f}"),
    ]
    for i, (field_name, expected) in enumerate(expected_values):
        verify_equals(
            actual_trade_order_info[i], expected,
            f"Verify {field_name!r} is correct in confirmation popup"
        )

    time_fmt = "%Y-%m-%d %H:%M:%S"
    server_time = datetime.strptime(web.home_page.get_server_time(), time_fmt).replace(second=0, microsecond=0)

    logger.info(f"Step 4: Click 'Confirm' at server time: {server_time}")
    web.trade_page.trade_confirmation_popup.confirm()
    web.trade_page.wait_for_loading_complete()

    logger.info("Step 5: Focus on 'Open Position' area")
    actual_open_date = web.trade_page.asset_order.get_latest_open_date(AssetOrderType.OPEN_POSITIONS)
    verify_equals(
        datetime.strptime(actual_open_date, time_fmt).replace(second=0, microsecond=0), server_time,
        f"Verify {actual_open_date!r} is match the server time when placing the order"
    )

    methods = [
        web.trade_page.asset_order.get_latest_type,
        web.trade_page.asset_order.get_latest_units,
        web.trade_page.asset_order.get_latest_stop_loss,
        web.trade_page.asset_order.get_latest_take_profit,
    ]

    for i, (field_name, expected) in enumerate(expected_values):
        verify_equals(
            methods[i](AssetOrderType.OPEN_POSITIONS).lower(), expected.lower(),
            f"Verify {field_name!r} is correct in {AssetOrderType.OPEN_POSITIONS} area"
        )
