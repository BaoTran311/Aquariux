...
from src.data_object.trade_order import LimitTradeOrder
...
def test_place_buy_LIMIT_order(web, symbol):
    ...
    live_price = float(format_number_string(web.trade_page.place_order.get_live_buy_price()))

    logger.info("Step 3: Place buy Limit order")
    trade_order = LimitTradeOrder(
        OrderSide.BUY, {VolumeType.UNITS: 1}, live_price - 2, live_price - 5, live_price + 5,
    ...
    verify_equals(
        actual_trade_order_info[0], f"{trade_order.order_side} limit",
    ...
    verify_equals(
        ...
        "Verify 'limit price' is correct in confirmation popup"
    )
    verify_equals(
    ...
    verify_equals(
        web.trade_page.asset_order.get_latest_type(asset_tab), f"{trade_order.order_side} limit".upper(),
    ...
    verify_equals(
        ...
        f"Verify 'limit price' is correct in {asset_tab} area"
    )
    verify_equals(
